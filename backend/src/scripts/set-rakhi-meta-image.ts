...
/**
 * Set metadata.seo_image = the product's thumbnail for every product in the
 * "Rakhi" category — used by generateMetadata() (products/[handle]/page.tsx)
 * for the OG/Twitter share image.
 *
 * Usage (DRY RUN by default):
 *   npx medusa exec ./src/scripts/set-rakhi-meta-image.ts
 *   npx medusa exec ./src/scripts/set-rakhi-meta-image.ts apply
 */
export default async function run({ container, args }: ExecArgs) {
  ...
  const { data: categories } = await query.graph({
    ...
    fields: ["id", "name"],
    filters: { name: "Rakhi" } as any,
  })
  if (!categories.length) {
    logger.error(`No "Rakhi" category found.`)
    return
  }
  ...
  if (!products.length) {
    logger.error(`No products found in the "Rakhi" category.`)
    return
  }

  logger.info(
    `Found ${products.length} Rakhi product(s) — ${
